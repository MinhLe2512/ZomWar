...
public class CameraFit : MonoBehaviour
{
    ...
    private void ChangeViewForOrthographicCamera()
    {
        ...
        {
            case Mode.FixedHeight:
                cinemachineVirtualCamera.m_Lens.OrthographicSize = verticalSize;
                break;
            case Mode.FixedWidth:
                cinemachineVirtualCamera.m_Lens.OrthographicSize = horizontalSize / screenAspect;
                break;
            case Mode.Dynamic:
                cinemachineVirtualCamera.m_Lens.OrthographicSize = Mathf.Max(verticalSize, horizontalSize / screenAspect);
                break;
        }
        _cameraPrevSize = cinemachineVirtualCamera.m_Lens.OrthographicSize;
    }
    ...
    private void ChangeViewForPerspectiveCamera()
    {
        ...
        {
            case Mode.FixedHeight:
                cinemachineVirtualCamera.m_Lens.FieldOfView = verticalFOV;
                break;
            case Mode.FixedWidth:
                cinemachineVirtualCamera.m_Lens.FieldOfView = AngleConversionByRatio(horizontalFOV, 1 / screenAspect);
                break;
            case Mode.Dynamic:
                aspectDifference = Mathf.Min(1f, screenAspect / mainAreaAspectRatio);
                cinemachineVirtualCamera.m_Lens.FieldOfView = AngleConversionByRatio(verticalFOV, 1 / aspectDifference);
                break;
        }
        _cameraPrevFOV = cinemachineVirtualCamera.m_Lens.FieldOfView;
    }
}